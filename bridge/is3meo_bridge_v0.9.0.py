...
DEFAULTS = {
    ...
    'sender_url': 'https://kumodot.github.io/Is3meoVR/stream_sender/stream_sender_v0.8.0.html',
            ...
            for root, _dirs, files in os.walk(folder):
                for fn in files:
                    ...
                    items[iid] = {'id': iid, 'title': nice_title(fn), 'path': full, 'cat': cat.get('name', ''), 'mtime': mtime,
                                  'sbs': bool(re.search(r'\b(h?sbs|3d|h?ou|tab)\b', fn, re.I))}
                    ids.append(iid)
...
def library_for_quest(cfg, rescan=False):
    ...
    return {'t': 'library', 'players': players, 'player': (cfg.get('media') or {}).get('default_player', ''),
            'cats': [{'name': c['name'], 'items': [{'id': i, 'title': LIB['items'][i]['title'], 'sbs': LIB['items'][i]['sbs']} for i in c['ids']]}
                     for c in LIB['cats']]}
...
def thumb_for(iid):
    """JPEG (base64) for a library item: a poster next to the file, else a frame grabbed with ffmpeg."""
    import base64
...
def library_cmd(cfg, m):
    ...
    if c == 'thumbs':
        return [{'t': 'thumb', 'id': i, 'data': thumb_for(i)} for i in (m.get('ids') or [])[:24]]
    if c == 'play':
...
async def run(cfg):
    ...
    async with async_playwright() as pw:
        ...
        def save_config(new):
            try:
                allowed = ['code', 'capture_source', 'audio_device', 'bitrate', 'browser', 'apps', 'auto_open_app', 'speaker_mute', 'media']
                for k in allowed:
                    if k in new:
                        cfg[k] = new[k]
                code = str(cfg.get('code') or '')
