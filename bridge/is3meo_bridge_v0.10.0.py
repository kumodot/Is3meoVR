...
DEFAULTS = {
    ...
    'sender_url': 'https://kumodot.github.io/Is3meoVR/stream_sender/stream_sender_v0.9.0.html',
...
def library_for_quest(cfg, rescan=False):
    ...
    return {'t': 'library', 'players': players, 'player': (cfg.get('media') or {}).get('default_player', ''), 'cats': cats,
            'artwork': bool((cfg.get('artwork') or {}).get('tmdb_key'))}
...
def tmdb_get(cfg, path, **params):
    ...
    key = ((cfg.get('artwork') or {}).get('tmdb_key') or '').strip()
    if not key:
        return None
...
def art_for(cfg, entry_id):
    import base64
    info = art_meta(cfg, entry_id) if (cfg.get('artwork') or {}).get('tmdb_key') else None
    if not info:
...
def thumb_for(iid, cfg=None):
    ...
    import base64
    if cfg and (cfg.get('artwork') or {}).get('tmdb_key'):
        THUMBS_DIR.mkdir(exist_ok=True)
...
def library_cmd(cfg, m):
    ...
    if c == 'thumbs':
        return [{'t': 'thumb', 'id': i, 'data': thumb_for(i, cfg), 'poster': bool((cfg.get('artwork') or {}).get('tmdb_key'))} for i in (m.get('ids') or [])[:24]]
    if c == 'art':
