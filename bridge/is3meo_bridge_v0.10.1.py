...
VM_RESTORE_PATH = HERE / 'voicemeeter_restore.json'  # mute states to put back if the Bridge was closed while linked
THUMBS_DIR = HERE / 'thumbs'            # cached library thumbnails
...
DEFAULTS = {
    ...
    'sender_url': 'https://kumodot.github.io/Is3meoVR/stream_sender/stream_sender_v0.9.1.html',
    ...
    'apps': [
        {'id': 'stremio', 'name': 'Stremio', 'path': '', 'profile': 'stremio', 'fullscreen_key': 'F11', 'type': 'app', 'args': ''},
        {'id': 'potplayer', 'name': 'PotPlayer', 'path': '', 'profile': 'potplayer', 'fullscreen_key': 'Enter', 'type': 'player', 'args': '"{file}"'},
        {'id': 'vlc', 'name': 'VLC', 'path': '', 'profile': 'vlc', 'fullscreen_key': '', 'type': 'player', 'args': '--fullscreen "{file}"'},
    ],
    # Media library shown in VR: categories with folders. Players (type 'player') open the picked file.
    'media': {'categories': [{'name': 'Movies', 'folders': []}, {'name': 'TV Shows', 'folders': []}, {'name': 'Misc', 'folders': []}],
              'default_player': 'potplayer'},
    # Posters / backdrops like Kodi: TMDB key (v3 key or v4 read token), optional fanart.tv key for richer backdrops
    'artwork': {'tmdb_key': '', 'fanart_key': '', 'language': 'en-US'},
    'auto_open_app': 'stremio',         # opened (or brought to front) when the Quest connects ('' = off)
    # PC speakers while the Quest is linked. 'off' = leave them alone (default).
    # 'voicemeeter' = mute these VoiceMeeter hardware outputs (0 = A1, 1 = A2, ...) and put them back on disconnect.
    'speaker_mute': {'mode': 'off', 'buses': []},
...
def load_config():
    ...
    if cfg.get('config_version', 1) < 4:
        ...
        cfg['config_version'] = CONFIG_VERSION
    if cfg.get('config_version', 1) < 5:
        # v0.8.x: apps get a type (app / player) + open args, new media library block
        for e in cfg.get('apps') or []:
            d = next((x for x in DEFAULTS['apps'] if x['id'] == e.get('id')), None)
            e.setdefault('type', d['type'] if d else 'player')
            e.setdefault('args', d['args'] if d else '"{file}"')
        cfg['config_version'] = CONFIG_VERSION
...
def auto_open(cfg):
    ...
    if not app or app == 'library' or time.time() - _last_auto_open < 15:  # 'library' opens in VR, nothing to launch
        return
...
# ---------------------------------------------------------------------------
# ---------------------------------------------------------------------------
# VoiceMeeter (optional): mute the PC speakers while the Quest is linked
# The Quest keeps its sound: it comes from a virtual bus (e.g. B2 = "VoiceMeeter Aux Output"),
# which is separate from the hardware outputs A1..A5.
# ---------------------------------------------------------------------------
class VoiceMeeter:
    HW_OUTPUTS = {1: 1, 2: 3, 3: 5}  # VoiceMeeter type -> number of A outputs (Standard, Banana, Potato)

    def __init__(self):
        self.dll = None
        self.saved = {}   # bus index -> mute value before we touched it
    ...
    def _dll_path(self):
        try:
            ...
            with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, key) as k:
                folder = Path(winreg.QueryValueEx(k, 'UninstallString')[0].strip('"')).parent
                if (folder / 'VoicemeeterRemote64.dll').exists():
                    return folder / 'VoicemeeterRemote64.dll'
        except Exception:
            pass
        p = Path(r'C:\Program Files (x86)\VB\Voicemeeter\VoicemeeterRemote64.dll')
    ...
    def connect(self):
        ...
        try:
            ...
            print('VoiceMeeter API error:', e)
            return False

    def _type(self):
        t = ctypes.c_long(0)
        return t.value if self.dll.VBVMR_GetVoicemeeterType(ctypes.byref(t)) == 0 else 0

    def _get(self, name):
        self.dll.VBVMR_IsParametersDirty()
        f = ctypes.c_float(0)
        self.dll.VBVMR_GetParameterFloat(name.encode(), ctypes.byref(f))
        return f.value

    def _set(self, name, value):
        self.dll.VBVMR_SetParameterFloat(name.encode(), float(value))

    def info(self):
        if not self.connect():
            return {'available': False}
        n = self.HW_OUTPUTS.get(self._type(), 0)
        buses = []
        for i in range(n):
            buf = ctypes.create_unicode_buffer(512)
            self.dll.VBVMR_GetParameterStringW(f'Bus[{i}].device.name'.encode(), buf)
            buses.append({'index': i, 'name': f'A{i + 1}', 'device': buf.value, 'muted': self._get(f'Bus[{i}].Mute') >= 0.5})
        return {'available': True, 'type': {1: 'Voicemeeter', 2: 'Voicemeeter Banana', 3: 'Voicemeeter Potato'}.get(self._type(), '?'), 'buses': buses}

    def mute(self, buses):
        if not buses or not self.connect():
            return
        for b in buses:
            b = int(b)
            if b not in self.saved:
                self.saved[b] = self._get(f'Bus[{b}].Mute')
            self._set(f'Bus[{b}].Mute', 1)
        VM_RESTORE_PATH.write_text(json.dumps(self.saved), encoding='utf-8')
        print('VoiceMeeter: PC speakers muted (' + ', '.join(f'A{int(b) + 1}' for b in buses) + ')')

    def restore(self):
        if not self.saved or not self.connect():
            return
        for b, v in self.saved.items():
            self._set(f'Bus[{int(b)}].Mute', v)
        print('VoiceMeeter: PC speakers back to how they were')
        self.saved = {}
        try:
            VM_RESTORE_PATH.unlink()
        except FileNotFoundError:
            pass

    def restore_leftover(self):
        # The previous Bridge was closed while the Quest was linked: put its mute states back
        if VM_RESTORE_PATH.exists():
            if not self.connect() or not self._type():
                print('VoiceMeeter is not running: will put the PC speakers back once it is')
                return
            try:
                self.saved = {int(k): v for k, v in json.loads(VM_RESTORE_PATH.read_text(encoding='utf-8')).items()}
            except Exception:
                self.saved = {}
            self.restore()
            try:
...
def quest_linked(cfg, linked):
    sm = cfg.get('speaker_mute') or {}
    if linked and sm.get('mode') == 'voicemeeter':
        VM.mute(sm.get('buses') or [])
    elif not linked:
        VM.restore()


# ---------------------------------------------------------------------------
# Media library: folders per category -> list for the Quest, thumbnails, play with a player
# ---------------------------------------------------------------------------
VIDEO_EXT = {'.mp4', '.mkv', '.avi', '.mov', '.m4v', '.webm', '.ts', '.m2ts', '.wmv', '.flv', '.mpg', '.mpeg'}
LIB = {'items': {}, 'cats': [], 'time': 0, 'shows': {}}


def nice_title(name):
    t = re.sub(r'[._]+', ' ', Path(name).stem)
    t = re.sub(r'\s*[\[(]?(1080p|2160p|720p|4k|x264|x265|h264|h265|hevc|bluray|web-?dl|webrip|hdr|10bit).*$', '', t, flags=re.I)
    return t.strip() or Path(name).stem


def parse_name(fn):
    """'Show.Name.S01E02.x264.mkv' -> tv episode; 'Movie.Name.2017.1080p.mkv' -> movie + year."""
    stem = re.sub(r'[._]+', ' ', Path(fn).stem)
    m = re.search(r'\b[Ss](\d{1,2})[ ._-]?[Ee](\d{1,3})\b|\b(\d{1,2})x(\d{2})\b', stem)
    if m:
        show = re.sub(r'[\s(\[-]+$', '', stem[:m.start()]).strip()
        yr = re.search(r'\b(19|20)\d{2}\b', show)
        if yr:
            show = show[:yr.start()].strip() or show
        return {'kind': 'tv', 'show': show or stem, 'season': int(m.group(1) or m.group(3)), 'episode': int(m.group(2) or m.group(4)),
                'epname': nice_title(stem[m.end():]) if stem[m.end():].strip() else ''}
    ys = [y for y in re.finditer(r'\b(19[3-9]\d|20[0-4]\d)\b', stem) if y.start() > 0]
    y = ys[-1] if ys else None  # last year: 'Blade Runner 2049 2017' -> 2017
    if y:
        return {'kind': 'movie', 'mtitle': stem[:y.start()].strip(' ([-'), 'year': int(y.group(1))}
    return {'kind': 'movie', 'mtitle': nice_title(fn), 'year': 0}


def scan_library(cfg):
    import hashlib
    items, cats = {}, []
    for cat in (cfg.get('media') or {}).get('categories') or []:
        ids = []
        for folder in cat.get('folders') or []:
            ...
            for root, _dirs, files in os.walk(folder):
                for fn in files:
                    if Path(fn).suffix.lower() not in VIDEO_EXT or len(ids) >= 3000:
                        continue
                    full = os.path.join(root, fn)
                    iid = hashlib.sha1(full.encode('utf-8', 'ignore')).hexdigest()[:12]
                    try:
                        mtime = os.path.getmtime(full)
                    except OSError:
                        mtime = 0
                    items[iid] = {'id': iid, 'title': nice_title(fn), 'path': full, 'cat': cat.get('name', ''), 'mtime': mtime,
                                  'sbs': bool(re.search(r'\b(h?sbs|3d|h?ou|tab)\b', fn, re.I)), **parse_name(fn)}
                    ids.append(iid)
        ids.sort(key=lambda i: items[i]['title'].lower())
        cats.append({'name': cat.get('name', ''), 'ids': ids})
    LIB.update(items=items, cats=cats, time=time.time())
    print(f'Library: {len(items)} videos in {len(cats)} categories')
    return LIB


def library_for_quest(cfg, rescan=False):
    if rescan or not LIB['time'] or time.time() - LIB['time'] > 300:
        scan_library(cfg)
    players = [{'id': e['id'], 'name': e['name']} for e in cfg.get('apps', []) if e.get('type') == 'player']
    cats = []
    for c in LIB['cats']:
        entries, shows = [], {}
        for i in c['ids']:
            it = LIB['items'][i]
            if it.get('kind') == 'tv':
                key = 'show:' + re.sub(r'[^a-z0-9]+', '', it['show'].lower())[:40]
                sh = shows.get(key)
                if not sh:
                    sh = shows[key] = {'id': key, 'title': it['show'], 'kind': 'tv', 'episodes': []}
                    entries.append(sh)
                    LIB['shows'][key] = it['show']
                sh['episodes'].append({'id': i, 'label': f"S{it['season']:02d}E{it['episode']:02d}" + (f" \u00b7 {it['epname']}" if it.get('epname') else ''),
                                       's': it['season'], 'e': it['episode']})
            else:
                entries.append({'id': i, 'title': it['title'], 'year': it.get('year', 0), 'sbs': it['sbs'], 'kind': 'movie'})
        for sh in shows.values():
            sh['episodes'].sort(key=lambda e: (e['s'], e['e']))
        entries.sort(key=lambda e: e['title'].lower())
        cats.append({'name': c['name'], 'items': entries})
    return {'t': 'library', 'players': players, 'player': (cfg.get('media') or {}).get('default_player', ''), 'cats': cats,
            'artwork': bool(tmdb_key(cfg))}
...
# ---- TMDB / fanart.tv artwork (cached in bridge/thumbs) ----
META_PATH = THUMBS_DIR / 'meta.json'
# Built-in TMDB key of this project (read-only, like Kodi ships its own). Users can set their own in Settings.
DEFAULT_TMDB_KEY = 'c1abe072661577082d4fc9b213002ff5'


def tmdb_key(cfg):
    return ((cfg.get('artwork') or {}).get('tmdb_key') or '').strip() or DEFAULT_TMDB_KEY
_META = None
...
def _http(url, headers=None, timeout=10):
    ...
    req = urllib.request.Request(url, headers={'User-Agent': 'Is3meoBridge/' + VERSION, **(headers or {})})
    with urllib.request.urlopen(req, timeout=timeout) as r:
        return r.read()


def tmdb_get(cfg, path, **params):
    from urllib.parse import urlencode
    key = tmdb_key(cfg)
    params['language'] = (cfg.get('artwork') or {}).get('language') or 'en-US'
    hdr = {}
    if len(key) > 40:
        hdr['Authorization'] = 'Bearer ' + key  # v4 read access token
    else:
        params['api_key'] = key
    return json.loads(_http(f'https://api.themoviedb.org/3{path}?{urlencode(params)}', hdr))


def art_meta(cfg, entry_id):
    """TMDB match for a movie item or a 'show:' entry: {title, year, overview, poster, backdrop, tmdb, kind}."""
    if entry_id.startswith('show:'):
        kind, title, year = 'tv', LIB['shows'].get(entry_id, ''), 0
    else:
        it = LIB['items'].get(entry_id)
        if not it:
            return None
        kind, title, year = 'movie', it.get('mtitle') or it['title'], it.get('year', 0)
    mkey = f'{kind}|{title.lower()}|{year}'
    cache = _meta()
    if mkey in cache:
        return cache[mkey]
    info = None
    try:
        q = {'query': title}
        if year:
            q['year' if kind == 'movie' else 'first_air_date_year'] = year
        res = (tmdb_get(cfg, f'/search/{kind}', **q) or {}).get('results') or []
        if not res and year:
            res = (tmdb_get(cfg, f'/search/{kind}', query=title) or {}).get('results') or []
        if res:
            r = res[0]
            date = r.get('release_date') or r.get('first_air_date') or ''
            info = {'kind': kind, 'tmdb': r['id'], 'title': r.get('title') or r.get('name') or title, 'year': int(date[:4]) if date[:4].isdigit() else 0,
                    'overview': (r.get('overview') or '')[:400], 'poster': r.get('poster_path') or '', 'backdrop': r.get('backdrop_path') or ''}
            fk = ((cfg.get('artwork') or {}).get('fanart_key') or '').strip()
            if fk:
                try:
                    if kind == 'movie':
                        fa = json.loads(_http(f"https://webservice.fanart.tv/v3/movies/{r['id']}?api_key={fk}"))
                        bg = fa.get('moviebackground') or []
                    else:
                        tvdb = (tmdb_get(cfg, f"/tv/{r['id']}/external_ids") or {}).get('tvdb_id')
                        fa = json.loads(_http(f'https://webservice.fanart.tv/v3/tv/{tvdb}?api_key={fk}')) if tvdb else {}
                        bg = fa.get('showbackground') or []
                    if bg:
                        info['fanart'] = bg[0]['url']
                except Exception:
                    pass
    except Exception as e:
        print('Artwork lookup failed:', title, e)
        return None  # not cached: try again next time (network / key problem)
    cache[mkey] = info
...
def _cached_image(url, out):
    if not out.exists():
        try:
            out.write_bytes(_http(url, timeout=15))
        except Exception:
            return None
    return out


def poster_for(cfg, entry_id):
    info = art_meta(cfg, entry_id)
    if info and info.get('poster'):
        return _cached_image('https://image.tmdb.org/t/p/w342' + info['poster'], THUMBS_DIR / f"tmdb_{info['kind']}_{info['tmdb']}_p.jpg")
    return None


def art_for(cfg, entry_id):
    import base64
    info = art_meta(cfg, entry_id) if tmdb_key(cfg) else None
    if not info:
        return {'t': 'art', 'id': entry_id}
    img = None
    if info.get('fanart'):
        img = _cached_image(info['fanart'].replace('/fanart/', '/preview/'), THUMBS_DIR / f"fanart_{info['kind']}_{info['tmdb']}_b.jpg")
    if not img and info.get('backdrop'):
        img = _cached_image('https://image.tmdb.org/t/p/w780' + info['backdrop'], THUMBS_DIR / f"tmdb_{info['kind']}_{info['tmdb']}_b.jpg")
    return {'t': 'art', 'id': entry_id, 'title': info['title'], 'year': info['year'], 'overview': info['overview'],
            'backdrop': base64.b64encode(img.read_bytes()).decode() if img else None}


def thumb_for(iid, cfg=None):
    """JPEG (base64): TMDB poster, else a poster next to the file, else a frame grabbed with ffmpeg."""
    import base64
    if cfg and tmdb_key(cfg):
        THUMBS_DIR.mkdir(exist_ok=True)
        p = poster_for(cfg, iid)
        if p:
            return base64.b64encode(p.read_bytes()).decode()
    if iid.startswith('show:'):
        eps = [i for i, it in LIB['items'].items() if it.get('kind') == 'tv' and 'show:' + re.sub(r'[^a-z0-9]+', '', it['show'].lower())[:40] == iid]
        if not eps:
            return None
        iid = eps[0]
    it = LIB['items'].get(iid)
    if not it:
        return None
    out = THUMBS_DIR / f'{iid}.jpg'
    if not out.exists():
        THUMBS_DIR.mkdir(exist_ok=True)
        src = Path(it['path'])
        poster = next((p for p in [src.with_suffix('.jpg'), src.with_suffix('.png'), src.parent / 'poster.jpg', src.parent / 'folder.jpg', src.parent / 'cover.jpg']
                       if p.exists()), None)
        ff = ffmpeg_exe()
        if not ff:
            if poster and poster.suffix.lower() == '.jpg' and poster.stat().st_size < 400000:
                return base64.b64encode(poster.read_bytes()).decode()
            return None
        vf = 'scale=320:180:force_original_aspect_ratio=increase,crop=320:180'
        for ss in ([] if poster else ['300', '30', '1']) or [None]:
            cmd = [ff, '-v', 'error', '-y'] + (['-ss', ss] if ss else []) + ['-i', str(poster or src), '-frames:v', '1', '-vf', vf, '-q:v', '5', str(out)]
            try:
                subprocess.run(cmd, capture_output=True, timeout=20, creationflags=0x08000000)
            except Exception:
                pass
            if out.exists() and out.stat().st_size > 0:
                break
    return base64.b64encode(out.read_bytes()).decode() if out.exists() else None


def play_item(cfg, iid, app):
    it = LIB['items'].get(iid)
    if not it:
        return {'t': 'playing', 'ok': False, 'error': 'Not in the library (rescan?)'}
    app = app or (cfg.get('media') or {}).get('default_player') or ''
    entry = app_entry(cfg, app)
    exe = find_app(cfg, app)
    if not exe:
        return {'t': 'playing', 'ok': False, 'error': f'{app} not found: set its exe in Settings'}
    import shlex
    args = [a.replace('{file}', it['path']) for a in shlex.split(entry.get('args') or '"{file}"', posix=False)]
    args = [a[1:-1] if len(a) > 1 and a[0] == a[-1] == '"' else a for a in args]
    print(f'Playing {it["title"]} with {entry.get("name", app)}')
    subprocess.Popen([exe] + args, cwd=str(Path(exe).parent))
    threading.Thread(target=bring_to_front, args=(cfg, app, exe), daemon=True).start()
    return {'t': 'playing', 'ok': True, 'app': app, 'profile': entry.get('profile') or app, 'title': it['title'], 'sbs': it['sbs']}


def library_cmd(cfg, m):
    """Library messages from the Quest; returns the replies to send back."""
    c = m.get('c')
    if c == 'library':
        return [library_for_quest(cfg, bool(m.get('rescan')))]
    if c == 'thumbs':
        return [{'t': 'thumb', 'id': i, 'data': thumb_for(i, cfg), 'poster': bool(tmdb_key(cfg))} for i in (m.get('ids') or [])[:24]]
    if c == 'art':
        return [art_for(cfg, m.get('id') or '')]
    if c == 'play':
        return [play_item(cfg, m.get('id'), m.get('app'))]
    return []
...
async def run(cfg):
    ...
    async with async_playwright() as pw:
        ...
        def save_config(new):
            try:
                allowed = ['code', 'capture_source', 'audio_device', 'bitrate', 'browser', 'apps', 'auto_open_app', 'speaker_mute', 'media', 'artwork']
                for k in allowed:
                    if k in new:
                        cfg[k] = new[k]
                code = str(cfg.get('code') or '')
        ...
        def vm_test(buses):
            # Mute for 3 seconds so you can hear which output is which
            VM.mute(buses)
            threading.Timer(3.0, VM.restore).start()
            return True
        ...
        def browse_folder(current=''):
            ...
            def pick():
                try:
                    ...
                    from tkinter import filedialog
                    root = tk.Tk(); root.withdraw(); root.attributes('-topmost', True)
                    result['path'] = filedialog.askdirectory(parent=root, title='Choose a media folder', initialdir=current or str(Path.home())) or ''
                    root.destroy()
                except Exception as e:
                    result['error'] = str(e)
        ...
        async def lib_cmd(m):
            # scanning / ffmpeg can take a while: keep the page responsive
            return await asyncio.get_running_loop().run_in_executor(None, library_cmd, cfg, m)

        def lib_stats():
            scan_library(cfg)
            return {'cats': [{'name': c['name'], 'count': len(c['ids'])} for c in LIB['cats']], 'ffmpeg': bool(ffmpeg_exe())}

        await page.expose_function('is3meoBrowseFolder', browse_folder)
        await page.expose_function('is3meoLibraryCmd', lib_cmd)
        await page.expose_function('is3meoLibraryStats', lib_stats)
        await page.expose_function('is3meoVoicemeeterInfo', lambda: VM.info())
        await page.expose_function('is3meoVoicemeeterTest', vm_test)
        await page.expose_function('is3meoQuestLinked', lambda on: quest_linked(cfg, bool(on)))
